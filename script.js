                ...
                <table style="width: 100%; text-align: left; border-collapse: collapse;">
                    <thead>
                        <tr style="border-bottom: 2px solid var(--border); color: #64748b; font-size: 0.8rem;">
                            <th style="padding: 12px 8px;">Requested Equipment</th>
                            <th style="padding: 12px 8px; text-align:center; width: 150px;">Requested Qty</th>
                            <th style="padding: 12px 8px; text-align:center; width: 220px;">Individual Action</th>
                        </tr>
                    </thead>
...
async function approveAllGlobal() {
    let pendingItems = vaultData.filter(i => i.status === 'Pending Approval');
    
    if (pendingItems.length === 0) {
        return alert("There are no pending requests to approve.");
    }

    if (!confirm(`Are you sure you want to approve ALL ${pendingItems.length} pending items across all students?`)) return;
    ...
    addLog(`Globally approved ${pendingItems.length} pending items`, "SUCCESS", "Admin");
    
    const itemsRes = await fetch(`${API_URL}/items`);
    vaultData = await itemsRes.json();
    applyFilters();
    alert(`Successfully approved ${pendingItems.length} items.`);
}

async function rejectAllGlobal() {
    let pendingItems = vaultData.filter(i => i.status === 'Pending Approval');
    
    if (pendingItems.length === 0) {
        return alert("There are no pending requests to reject.");
    }

    if (!confirm(`CRITICAL ACTION: Are you sure you want to completely REJECT ALL ${pendingItems.length} pending requests across the entire system?`)) return;
    ...
    addLog(`Globally rejected ${pendingItems.length} pending items`, "DELETED", "Admin");
    
    const itemsRes = await fetch(`${API_URL}/items`);
    vaultData = await itemsRes.json();
    applyFilters();
    alert(`Successfully rejected and restored ${pendingItems.length} items.`);
}
...
function updateChart() {
    ...
    if (ctxPop) {
        ...
        auditLogs.forEach(log => {
            if (log.action.startsWith("Approved individual item: ")) {
                let eqName = log.action.replace("Approved individual item: ", "").trim();
...
async function removeItem(id) {
    if(confirm("Delete this entire group of items?")) {
        ...
        if (itemToDelete) {
            addLog(`Permanently Deleted Group: ${itemToDelete.equipment}`, "DELETED", "Admin");
        }
        applyFilters();
    }
}
